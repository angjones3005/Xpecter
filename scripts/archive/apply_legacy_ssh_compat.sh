...
type Config struct {
	...
	// LegacyCompat (SPE-99) widens the negotiated cipher/KEX/MAC/host-key
	// algorithm sets to include older options golang.org/x/crypto/ssh
	// excludes by default (CBC-mode ciphers, SHA-1 KEX, plain ssh-rsa,
	// etc.), for older/embedded network devices that only speak those.
	// Explicit per-connection opt-in, never a global default change,
	// see the Dial() comment below for why.
	LegacyCompat bool
}
...
func Dial(cfg Config) (*Session, error) {
	...
	if cfg.LegacyCompat {
		// SPE-99: combine the library's own real "modern" and "insecure"
		// algorithm lists rather than hardcoding a guessed copy of
		// either, that guess could silently drift from what the
		// installed x/crypto/ssh version actually implements/defaults
		// to. Modern algorithms listed first, so they're still
		// preferred whenever the peer supports them, legacy ones only
		// come into play as a fallback for older devices that need
		// them, confirmed real via a real CBC-only device tonight.
		...
		sshCfg.HostKeyAlgorithms = append(append([]string{}, supported.HostKeys...), insecure.HostKeys...)
	}
	...
	sess := &Session{id: newID(), client: client}
...
type ConnectRequest struct {
	...
	// LegacyCompat (SPE-99): explicit per-connection opt-in from the
	// New Session picker, widens negotiated algorithms for older
	// devices. Never a global default, see sshclient.Config.LegacyCompat.
...
export interface ConnectRequest {
  ...
  // SPE-99: explicit per-connection opt-in, widens negotiated SSH
  // algorithms for older/embedded devices.
  ...
  <div id="session-picker-overlay">
    <div id="session-picker">
      ...
      <div id="picker-ssh-fields" style="display:none;padding:16px;display:none;flex-direction:column;gap:8px;">
        ...
        <label style="font-size:12px;color:var(--text-dim);display:flex;align-items:center;gap:6px;" title="Widens accepted ciphers/key-exchange/MACs for older devices. Weaker security, only use if the connection fails without it.">
          <input type="checkbox" id="legacy-compat-toggle" /> Legacy device compatibility mode
        </label>
...
document.getElementById('connect')!.addEventListener('click', async () => {
  ...
  // SPE-99: read once here, at the initial request build. Retries after
  // a passphrase/trust/key-perm prompt spread {...req, ...} elsewhere,
  // so this value carries through automatically without needing to be
  // re-read at each retry site.
  const legacyCompat = (document.getElementById('legacy-compat-toggle') as HTMLInputElement).checked;
  ...
  if (authMode === 'key') {
    ...
    req = { host, port: 22, user, keyPath, passphrase, legacyCompat };
  } else {
    const password = (document.getElementById('password') as HTMLInputElement).value;
    req = { host, port: 22, user, password, legacyCompat };
  }
...
echo "Wrote $(wc -l < frontend/src/main.ts) lines to main.ts (expect 2669)"
